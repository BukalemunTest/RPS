...
for _ in range(nesne_sayisi):
    tur = random.choice(["tas", "kagit", "makas"])
    nesne = Nesne(random.randint(0, ekran_genislik), random.randint(0, ekran_yukseklik), tur)

    # Olası çakışmaları kontrol et
    while any(carpisti_mi(nesne, diger_nesne) for diger_nesne in nesneler):
        nesne.x = random.randint(0, ekran_genislik - nesne.genislik)
        nesne.y = random.randint(0, ekran_yukseklik - nesne.yukseklik)
